...
class FindBallStateMachine:
    ...
    def start_rotation_and_forwarding(self, event=None):
        ...
        if self.agent.get_ball_history()[-1] != [] and self.agent.get_ball_history()[-2] != [] and time.time() - self.agent.get_ball_history()[-1]['timestamp'] < 5:
            # 计算最近2帧球相对位置y坐标的平均值
            ball_dir = self.agent.get_ball_history()[-1]['pos'][0] + self.agent.get_ball_history()[-2]['pos'][0]
